    ...
    try {
      const { rows: orders } = await sql`
        SELECT id, stripe_session_id, customer_email, amount_total, currency, status,
               tracking_carrier, tracking_number, shipped_at, delivered_at, created_at
        FROM orders
  ...
  if (req.method === "PATCH") {
    ...
    try {
      const { rows: existingRows } = await sql`
        SELECT customer_email, tracking_number, status, delivered_at FROM orders WHERE id = ${orderId};
      `;
      ...
      const isNewlyDelivered = status === "delivered" && previous.status !== "delivered" && !previous.delivered_at;
      ...
      const { rows } = await sql`
        UPDATE orders SET
          ...
          delivered_at = CASE WHEN ${isNewlyDelivered} THEN now() ELSE delivered_at END
        WHERE id = ${orderId}
        RETURNING id, customer_email, status, tracking_carrier, tracking_number, shipped_at, delivered_at;
      `;
      ...
      if (isNewlyDelivered) {
        await sendMail({
          to: updated.customer_email,
          subject: "Votre commande Ælen Paris est arrivée",
          html: `
            <p>Bonjour,</p>
            <p>Votre commande n°${updated.id} a été livrée. Nous espérons qu'elle vous plaît !</p>
            <p>À très vite,<br>L'équipe Ælen Paris</p>
          `,
        });
      }
